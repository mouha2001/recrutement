                                                            ...
                                                            @if(!isset($displayedCandidates[$candidat->id]))
                                                                ...
                                                                @endphp
                                                                    <tr class="odd:bg-white odd:dark:bg-white-900 even:bg-gray-50 even:dark:bg-white-800 border-b dark:border-gray-700">
                                                                        ...
                                                                        <td class="px-6 py-4">
                                                                            <a href="{{ route('voirpluspresidentchimie', ['postuler_id' => $post->id , 'id' => $candidat->id]) }}" class="text-blue-600 dark:text-blue-500 hover:underline transition duration-300 ease-in-out">Voir plus</a>
                                                                        </td>
                                                                        <td>
                                                        @if (!$candidat->pre_selected)
                                                            <a href="{{ route('preSelectionnerchimie', ['id' => $candidat->id]) }}" onclick="event.preventDefault(); document.getElementById('preSelectionnerchimie-form-{{ $candidat->id }}').submit();" class="text-green-600 dark:text-green-500 hover:underline transition duration-300 ease-in-out ml-2">Pré-sélectionner</a>
                                                            <form id="preSelectionnerchimie-form-{{ $candidat->id }}" action="{{ route('preSelectionnerchimie', ['id' => $candidat->id]) }}" method="POST" style="display: none;">
                                                                @csrf
                                                            </form>
                                                        @else
                                                            <a href="{{ route('deselectionnerchimie', ['id' => $candidat->id]) }}" onclick="event.preventDefault(); document.getElementById('deselectionnerchimie-form-{{ $candidat->id }}').submit();" class="text-blue-600 dark:text-blue-500 hover:underline transition duration-300 ease-in-out ml-2">Désélectionner</a>
                                                            <form id="deselectionnerchimie-form-{{ $candidat->id }}" action="{{ route('deselectionner', ['id' => $candidat->id]) }}" method="POST" style="display: none;">
                                                                @csrf
                                                            </form>
                                                        @endif
                                                    </td>
