        ...
        <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50">
            <ul class="space-y-4 font-medium">
                ...
                <li>
                  ...
                  <li>
                    <a href="{{ route('intersante') }}" class="flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 group">
                        <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="black" viewBox="0 0 24 24">
                            <path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M4.5 17H4a1 1 0 0 1-1-1 3 3 0 0 1 3-3h1m0-3.05A2.5 2.5 0 1 1 9 5.5M19.5 17h.5a1 1 0 0 0 1-1 3 3 0 0 0-3-3h-1m0-3.05a2.5 2.5 0 1 0-2-4.45m.5 13.5h-7a1 1 0 0 1-1-1 3 3 0 0 1 3-3h3a3 3 0 0 1 3 3 1 1 0 0 1-1 1Zm-1-9.5a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0Z" />
                        </svg>
                                                        ...
                                                        @if(!isset($displayedCandidates[$candidat->id]))
                                                            ...
                                                            @endphp
                                                                <tr class="odd:bg-white odd:dark:bg-white-900 even:bg-gray-50 even:dark:bg-white-800 border-b dark:border-gray-700">
                                                                    ...
                                                                    <td class="px-6 py-4">
                                                                        <a href="{{ route('voirpluspresidentsante', ['postuler_id' => $post->id , 'id' => $candidat->id]) }}" class="text-blue-600 dark:text-blue-500 hover:underline transition duration-300 ease-in-out">Voir plus</a>
                                                                    </td>
                                                                         <td>
                                                                            @if (!$candidat->pre_selected)
                                                                                <a href="{{ route('preSelectionnersante', ['id' => $candidat->id]) }}" onclick="event.preventDefault(); document.getElementById('preSelectionnersante-form-{{ $candidat->id }}').submit();" class="text-green-600 dark:text-green-500 hover:underline transition duration-300 ease-in-out ml-2">Pré-sélectionner</a>
                                                                                <form id="preSelectionnersante-form-{{ $candidat->id }}" action="{{ route('preSelectionnersante', ['id' => $candidat->id]) }}" method="POST" style="display: none;">
                                                                                    @csrf
                                                                                </form>
                                                                            @else
                                                                                <a href="{{ route('deselectionnersante', ['id' => $candidat->id]) }}" onclick="event.preventDefault(); document.getElementById('deselectionnersante-form-{{ $candidat->id }}').submit();" class="text-blue-600 dark:text-blue-500 hover:underline transition duration-300 ease-in-out ml-2">Désélectionner</a>
                                                                                <form id="deselectionnersante-form-{{ $candidat->id }}" action="{{ route('deselectionnersante', ['id' => $candidat->id]) }}" method="POST" style="display: none;">
                                                                                    @csrf
                                                                                </form>
                                                                            @endif
                                                                        </td>
