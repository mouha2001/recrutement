                ...
                <div class="flex flex-col items-center mb-3 mt-32 mx-5">
                    <div class="relative overflow-auto shadow-md sm:rounded-lg mb-8 mt-4 w-full">
                        @foreach($posts as $post)
                            @if ($post->postes =='per'  )
                                @if ( $post->departements =='management')
                                                        ...
                                                        @if(!isset($displayedCandidates[$candidat->id]))
                                                            ...
                                                            @endphp
                                                                <tr class="odd:bg-white odd:dark:bg-white-900 even:bg-gray-50 even:dark:bg-white-800 border-b dark:border-gray-700">
                                                                    ...
                                                                    <td class="px-6 py-4">
                                                                        <a href="{{ route('voirpluspresidentmanagement', ['postuler_id' => $post->id , 'id' => $candidat->id]) }}" class="text-blue-600 dark:text-blue-500 hover:underline transition duration-300 ease-in-out">Voir plus</a>
                                                                    </td>
                                                                          <td>
                                                                            @if (!$candidat->pre_selected)
                                                                                <a href="{{ route('preSelectionnermanagement', ['id' => $candidat->id]) }}" onclick="event.preventDefault(); document.getElementById('preSelectionnermanagement-form-{{ $candidat->id }}').submit();" class="text-green-600 dark:text-green-500 hover:underline transition duration-300 ease-in-out ml-2">Pré-sélectionner</a>
                                                                                <form id="preSelectionnermanagement-form-{{ $candidat->id }}" action="{{ route('preSelectionnermanagement', ['id' => $candidat->id]) }}" method="POST" style="display: none;">
                                                                                    @csrf
                                                                                </form>
                                                                            @else
                                                                                <a href="{{ route('deselectionnermanagement', ['id' => $candidat->id]) }}" onclick="event.preventDefault(); document.getElementById('deselectionnermanagement-form-{{ $candidat->id }}').submit();" class="text-blue-600 dark:text-blue-500 hover:underline transition duration-300 ease-in-out ml-2">Désélectionner</a>
                                                                                <form id="deselectionnermanagement-form-{{ $candidat->id }}" action="{{ route('deselectionnermanagement', ['id' => $candidat->id]) }}" method="POST" style="display: none;">
                                                                                    @csrf
                                                                                </form>
                                                                            @endif
                                                                        </td>
